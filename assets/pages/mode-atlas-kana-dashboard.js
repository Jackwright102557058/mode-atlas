...
(function(){
  ...
  function renderPresetChecklist(){
    ...
    panel.innerHTML=`
      ...
      <div class="ma-achievement-grid">${M().PRESET_TRACKERS.map(p=>{ const n=Math.min(100,p.chars.reduce((sum,ch)=>sum+M().charCorrect(ch),0)); const done=n>=100; return `<article class="ma-achievement-card ${done?'done':''}"><div class="ma-achievement-top"><b>${p.name}</b><span>${n}/100</span></div><small>${p.desc}</small><div class="ma-progress-track"><span style="width:${n}%"></span></div><em>${done?'Complete':'In progress'}</em></article>`; }).join('')}</div>`;
  ...
  function renderSpeedMastery(afterEl){
    ...
    panel.innerHTML=`
      ...
      <div class="ma-speed-grid"><div class="ma-speed-card"><span class="label">Goal pace</span><strong>${fast}</strong><small>kana under 2.0s</small><div class="ma-meter"><i style="width:${pct(fast)}%"></i></div></div><div class="ma-speed-card"><span class="label">Fluent pace</span><strong>${fluent}</strong><small>kana under 1.0s</small><div class="ma-meter"><i style="width:${pct(fluent)}%"></i></div></div><div class="ma-speed-card"><span class="label">Mastered</span><strong>${counts.Mastered}</strong><small>accurate, repeated, and near target speed</small><div class="ma-meter"><i style="width:${pct(counts.Mastered)}%"></i></div></div><div class="ma-speed-card"><span class="label">Needs attention</span><strong>${counts.Learning+counts.Reviewing}</strong><small>${counts.Reviewing} reviewing · ${counts.Learning} learning</small></div></div>
